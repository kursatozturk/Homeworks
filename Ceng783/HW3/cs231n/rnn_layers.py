...
def rnn_forward(x, h0, Wx, Wh, b):
  ...
  for t in xrange(T):
    x_t = x[:, t]
    next_h, c = rnn_step_forward(x_t, prev_h, Wx, Wh, b)
    h[:, t] = next_h
    prev_h = next_h
    cache.append(c)
    ##############################################################################
...
def rnn_backward(dh, cache):
  ...
  for c in reversed(cache):
    t -= 1
    h_t = dh[:, t] + dh0
    dxt, dprev_ht, dWxt, dWht, dbt = rnn_step_backward(h_t, c)
    dx[:,t] = dxt
    dh0 = dprev_ht
    dWx += dWxt
    dWh += dWht
...
def lstm_forward(x, h0, Wx, Wh, b):
  ...
  for t in xrange(T):
    x_t = x[:, t]
    next_h, next_c, ca = lstm_step_forward(x_t, prev_h, prev_c, Wx, Wh, b)
    h[:, t] = next_h
    prev_h = next_h
    prev_c = next_c
    cache.append(ca)
...
def lstm_backward(dh, cache):
  ...
  for t in reversed(range(T)):
    c = cache[t]
    h = dh[:, t] + tdprev_h
    tdx, tdprev_h, tdprev_c, tdWx, tdWh, tdb = lstm_step_backward(h, dn_c, c)
    dx[:, t] = tdx
    dn_c = tdprev_c
    dh0 = tdprev_h
    dWx += tdWx
