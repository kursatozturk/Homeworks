...
public:
     ...
     */
    Blockchain(int id): id(id) ,head(nullptr), softforked(false), softforkedchains(nullptr){ };
    /**
     ...
     */
    Blockchain(int id, Koin *head): id(id), head(head), softforked(false), softforkedchains(nullptr) { };
    /**
     * Copy Constructor.
     *
     ...
     */
    Blockchain(const Blockchain& rhs):id(rhs.id), softforked(rhs.softforked), softforkedchains(rhs.softforkedchains) {
        Koin *p(rhs.head);
        this->head = new Koin((rhs.head)->getValue());
        Koin *q(this->head);

        while(p->getNext()){
            q->setNext(new Koin(p->getNext()->getValue()));
            q = q->getNext();
            p = p->getNext();
        }
    };
    ...
    void softfork();
    Blockchain* softforkchain();
    void softforkchain(Blockchain*);
};
