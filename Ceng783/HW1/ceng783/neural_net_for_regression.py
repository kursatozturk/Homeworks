...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    #############################################################################
    los_score_derivative = (scores - y) # dL_i/dS_ij

    gradw2 = relu_1_o.T.dot(los_score_derivative)
    gradb2 = np.sum(los_score_derivative * 1, axis = 0) 
    tempw1 = np.where((relu_1_o > 0), los_score_derivative.dot(W2.T), 0)
    gradw1 = tempw1.T.dot(X).T
    gradb1 = np.sum(tempw1, axis = 0)
