...
def affine_backward(dout, cache):
  ...
  """
  x, w, b = cache
  flattened_x = x.reshape(x.shape[0], w.shape[0])
  dw = np.dot(dout.T, flattened_x).T
  ...
  for i in range(N):
    for j in range(C):
      for k in range(Hp):
        ...
        for l in range(Wp):
          ws = l * stride
          max_o = np.max(x[i, j, hs:hs + HH, ws:ws + WW])
          dx[i, j, hs:hs + HH, ws:ws + WW] += (x[i, j, hs:hs + HH, ws:ws + WW] == max_o) * dout[i, j, k, l]
          # 
          # FILL IN THIS PART 
          # 
